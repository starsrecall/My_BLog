...
> 注意2：case里如果没有break，那么程序会一直向下执行,这就是switch的穿透
...
> 注意：continue并没有使整个循环终止，是跳过接下来的语句，而break会跳出循环
...
数组的元素个数为： 10
数组首地址为： 0x332ffff910
数组中第一个元素地址为： 0x332ffff910
数组中第二个元素地址为： 0x332ffff914
```

> 注意，可能会报错==error: cast from 'int*' to 'int' loses precision [-fpermissive]==，
>
> 因为这是一个C语言风格的强制类型转换，将**指针**（64位，8字节）强制转换为 **`int`**（通常是32位，4字节），导致**精度丢失**。推荐直接使用**cout << "数组中第一个元素地址为： " << &arr[0] << endl;**
...
二维数组行数： 2
二维数组列数： 3
二维数组首地址：0x43bbdffc70
二维数组第一行地址：0x43bbdffc70
二维数组第二行地址：0x43bbdffc7c
二维数组第一个元素地址：0x43bbdffc70
二维数组第二个元素地址：0x43bbdffc74
```
